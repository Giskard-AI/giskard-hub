...
We are releasing a new version of the Hub UI that introduces audit logs, a new task system, enhanced scans, and improved UI and content.
...
**Tasks & Follow-up Workflow**
   ...
   * **Draft Test Linked to Tasks** - When you create a task from a bad test will propose to automatically mark it as Draft
   * **Hide Noisy Results** - Hide noisy or sensitive evaluation results while still tracking them via tasks
...
**Draft Conversations & Datasets**
   Control over test cases lifecycle:
   
   * **Draft/Published Toggle** - A toggle with a helpful explanation
   * **Draft Filter in Tables** - Draft filter added to dataset tables with conversations status labels
   * **Consistent Exclusion** - Draft conversations are excluded from evaluation runs
...
**Audit & History**
   Comprehensive tracking and visibility of all changes across the platform:
